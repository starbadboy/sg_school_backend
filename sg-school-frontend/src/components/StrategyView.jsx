  ...
  const generateStrategy = async () => {
    ...
    try {
      const requestData = {
        ...
        target_schools: selectedSchools.filter(s => s && s.name).map(s => s.name),
...
Selected Schools:
${selectedSchools.filter(s => s && s.name).map(s => `- ${s.name} (${s.distance || 'N/A'}km away)`).join('\n')}
...
Generated by SG School Finder
  ...
  const getCompetitivenessData = () => {
    return selectedSchools.map(school => ({
              name: school && school.name ? school.name.split(' ').slice(0, 2).join(' ') : 'Unknown', // Shorter names for charts
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-gray-900 text-lg">{school && school.name ? school.name : 'Unknown School'}</h3>
              ...
              <div className="space-y-8">
                ...
                <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-8 border border-purple-200">
                  ...
                  <div className="grid gap-4">
                    {selectedSchools.map((school, index) => {
                      const successRate = getSchoolSuccessRate(school)
                      const riskData = getRiskAssessmentData().find(r => school && school.name && r.name === school.name.split(' ').slice(0, 2).join(' '))
                      ...
                      return (
                        <div key={index} className="flex items-center justify-between p-4 bg-white rounded-xl border border-gray-200">
                          <div className="flex items-center space-x-4">
                            ...
                            <div>
                              <h5 className="font-bold text-gray-900">{school && school.name ? school.name : 'Unknown School'}</h5>
